...
def test_pca_compression_1():
    ...
    true_answer_2 = np.array(
        [201.1, 208.2, 213.2, 218.7, 223.3, 221.2, 233.5, 220.4, 224.9, 219.5]
    ).astype(np.float64)
    for i in range(true_answer_0.shape[1]):
        assert (
            np.sum(np.abs(answer[0][:, i] - true_answer_0[:, i])) < 1e-5
            or np.sum(np.abs(-answer[0][:, i] - true_answer_0[:, i])) < 1e-5
        )
        assert (
            np.sum(np.abs(answer[1][i] - true_answer_1[i])) < 1e-5
            or np.sum(np.abs(-answer[1][i] - true_answer_1[i])) < 1e-5
        )
